...
def build_model(model_name, model_path: None):
        ...
        if img_content.shape[0] > 0 and img_content.shape[1] > 0:
            ...
            for index in pbar:
                ...
                if action == "emotion":
                    ...
                    emotion_predictions = analysis_model["emotion"].predict(img_gray, verbose=0)[0, :]
                    ...
                    age_predictions = analysis_model["age"].predict(img_content, verbose=0)[0, :]
                    apparent_age = Age.findApparentAge(age_predictions)
                    # int cast is for exception - object of type 'float32' is not JSON serializable
                    obj["age"] = int(apparent_age)

                elif action == "gender":
                    gender_predictions = analysis_model["gender"].predict(img_content, verbose=0)[0, :]
                    obj["gender"] = {}
                    ...
                    race_predictions = analysis_model["race"].predict(img_content, verbose=0)[0, :]
...
def find(
    ...
    for representation in representations:
        identity = representation[0]
        label = os.path.splitext(os.path.basename(identity))[0]
    ...
    for target_img, target_region, _ in target_objs:
        ...
        # Add labels to the response object
        result_df["label"] = result_df["identity"].map(labels)
...
def represent(
    ...
    for img, region, confidence in img_objs:
        ...
        # represent
        if "keras" in str(type(predict_model)):
            # new tf versions show progress bar and it is annoying
            embedding = predict_model.predict(img, verbose=0)[0].tolist()
        else:
            # SFace and Dlib are not keras models and no verbose arguments
            embedding = predict_model.predict(img)[0].tolist()
